...
export function InputRoot({ error = false, ...rest }: InputRootProps) {
  return (
    <div
      data-error={error}
      className="group flex items-center bg-gray-800 h-12 border border-gray-600 rounded-xl px-4 gap-2 focus-within:border-gray-100 data-[error=true]:border-danger"
      {...rest}
    />
  )
}
